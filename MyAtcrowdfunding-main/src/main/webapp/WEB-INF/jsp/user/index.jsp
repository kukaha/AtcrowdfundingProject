...
//          同步请求   	window.location.href="${APP_PATH}/user/index.do?pageno="+pageno;
            		...
            		success : function(result){
            			...
            			if(result.success){
            				...
            				$.each(data,function(i,user){
                				...
                				content+='  <td>';
                				content+='	  <button type="button" onclick="assignRole('+user.id+')" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                				content+='	  <button type="button" onclick="updateBtn('+user.id+')" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
                				content+='	  <button type="button" onclick="removeBtn('+user.id+',\''+user.loginacct+'\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
                				content+='  </td>';
                				content+='</tr>';
            				});
            ...
            function removeBtn(id,loginacct){
            	layer.confirm("你确认要删除"+loginacct+"用户吗",{icon:3,title:'提示'},function(cindex){
...
//             	});
            	...
            	var jsonObj = {};
            	$.each(selectCheckbox,function(i,checkbox){
            		jsonObj["datas["+i+"].id"] = checkbox.id;
            		jsonObj["datas["+i+"].loginacct"] = checkbox.name;
   				...
   				layer.close(cindex);
            		$.ajax({
	            		...
	            		//data : idStr,
	            		data : jsonObj,
	            		url:"${APP_PATH}/user/doRemoveBatch.do",
	            		beforeSend : function(){
	            			return true;
	            		},
	            		success : function(result){
	            		...
	            		error : function(){
	            			layer.msg("批量删除用户失败",{time:1000,icon:5,shift:6});
	            		}
	            	})
           		},function(cindex){
       				//取消要做的事情
